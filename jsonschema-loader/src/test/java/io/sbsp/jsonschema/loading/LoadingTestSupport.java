    ...
    public static <S extends Schema, E extends Exception> E expectFailure(final Failure<S, E> failure) {
        try {
            ...
            failure.expectedSchemaLocation().ifPresent(pointer -> {
                if (e instanceof SchemaLoadingException) {
                    SchemaLoadingException error = (SchemaLoadingException) e;
                    assertThat(error.getReport().getIssues().stream()
                            .filter(err -> err.getLocation()
