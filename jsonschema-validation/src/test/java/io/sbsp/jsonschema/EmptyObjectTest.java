...
public class EmptyObjectTest {
    ...
    public void validateEmptyObject() {
        JsonObject jsonSubject = JsonUtils.readJsonObject("{\n" +
                ...
                "  \"properties\": {}\n" +
                "}");
        JsonObject schemaJson = ResourceLoader.resourceLoader().readJsonObject("json-schema-draft-06.json");

        Schema schema = SchemaLoaderImpl.schemaLoader(JsonProvider.provider()).readSchema(schemaJson);
