...
import static io.sbsp.jsonschema.validation.ValidationMocks.mockArraySchema;
...
public class ArraySchemaValidatorTest {
    ...
    public void additionalItemsSchema() {
        ...
        // return this;
        final Schema arraySchema = schemaBuilder().itemSchema(mockBooleanSchema())
    ...
    public void additionalItemsSchemaFailure() {
        SchemaBuilder nullSchema = schemaBuilder("nulls").type(NULL);
    ...
    @Before
    public void before() {
        arrayTestCases = resourceLoader().readJsonObject("arraytestcases.json");
    }

    @Test
    public void booleanItems() {
        Schema subject = schemaBuilder().allItemSchema(mockBooleanSchema()).build();
        expectFailure(subject, mockBooleanSchema().build(), "#/2", arrayTestCases.get("boolArrFailure"));
    }

    @Test
    public void doesNotRequireExplicitArray() {
        final Schema arraySchema = schemaBuilder()
    ...
    public void maxItems() {
        Schema subject = buildWithLocation(schemaBuilder().maxItems(0));
        failureOf(subject)
                .schema(subject)
                .expectedPointer("#")
    ...
    public void minItems() {
        Schema subject = buildWithLocation(schemaBuilder().minItems(2));
        failureOf(subject)
                .expectedPointer("#")
    ...
    public void nonUniqueArrayOfArrays() {
        Schema subject = buildWithLocation(schemaBuilder().needsUniqueItems(true));
        failureOf(subject)
                .expectedPointer("#")
    ...
    public void toStringAdditionalItems() {
        ...
        final JsonObject rawSchemaJson = loader.readObjectWithBuilder("tostring/arrayschema-list.json")
                .remove("items")
    ...
    public void toStringTest() {
        JsonObject rawSchemaJson = loader.readJsonObject("tostring/arrayschema-list.json");
        String actual = SchemaLoaderImpl.schemaLoader().readSchema(rawSchemaJson).toString();
        assertEquals(rawSchemaJson, readJsonObject(actual));
    }
    ...
    public void toStringTupleSchema() {
        JsonObject rawSchemaJson = loader.readJsonObject("tostring/arrayschema-tuple.json");
        String actual = SchemaLoaderImpl.schemaLoader().readSchema(rawSchemaJson).toString();
        assertEquals(rawSchemaJson, readJsonObject(actual));
    }
    ...
    public void tupleWithOneItem() {
        ...
        // itemSchemas.add(requireNonNull(itemSchema, "itemSchema cannot be null"));
        // return this;
        Draft6Schema subject = schemaBuilder().itemSchema(mockBooleanSchema()).build().asDraft6();
        final Schema expectedSchema = subject.getItemSchemas().get(0);
    ...
    public void uniqueItemsObjectViolation() {
        Schema subject = schemaBuilder().needsUniqueItems(true).build();
    ...
    public void uniqueItemsWithSameToString() {
        final Schema schema = schemaBuilder().needsUniqueItems(true).build();
    ...
    public void validate_WhenItemsSchemaHasEnum_AndArrayValueIsInEnumButWrongType_ThenFailWithTypeKeyword() {
        SchemaBuilder enumSchema = ValidationMocks.mockIntegerSchema()
        ...
        assertSoftly(a -> {
            a.assertThat(error.get().getKeyword()).isEqualTo(Keywords.TYPE);
    ...
    public void validate_WhenItemsSchemaHasEnum_ThenDontEnforceLexicalMatching() {
        SchemaBuilder enumSchema = mockNumberSchema()
                .enumValues(jsonArray(12, 24.3, 65));
    ...
    public void validate_WhenItemsSchemaHasEnum_ThenEnforceEachItem() {
        ...
        assertSoftly(assertj -> {
            ...
            assertj.assertThat(validationError.getKeyword()).isEqualTo(Keywords.ENUM);
