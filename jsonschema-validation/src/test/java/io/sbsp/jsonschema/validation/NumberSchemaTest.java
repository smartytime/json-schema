...
import static io.sbsp.jsonschema.validation.ValidationMocks.mockIntegerSchema;
...
public class NumberSchemaTest {
    ...
    private final ResourceLoader loader = resourceLoader();
    ...
    public void longNumber() {
        ...
        getValidator(schema).validate(jsonNumberValue(4278190207L));
    }
    ...
    public void notRequiresNumber() {
        final Schema numberSchema = mockSchema()
                .build();
        expectSuccess(() -> getValidator(numberSchema).validate(jsonStringValue("foo")));
    }

    @Test
    public void requiresIntegerSuccess() {
        final Schema numberSchema = mockNumberSchema().build();
        expectSuccess(() -> getValidator(numberSchema).validate(jsonNumberValue(10)));
    }
    ...
    public void smallMultipleOf() {
        final Schema schema = mockNumberSchema()
                .multipleOf(0.0001)
                .build();
        JsonSchemaProvider.getValidator(schema).validate(jsonNumberValue(0.0075));
    }
    ...
    public void success() {
        final Schema schema = mockNumberSchema()
                ...
                .multipleOf(10)
                .build();
        JsonSchemaProvider.getValidator(schema).validate(jsonNumberValue(10.0));
    }

    @Test
    public void toStringNoExplicitType() {
        JsonObject rawSchemaJson = loader.readObjectWithBuilder("tostring/numberschema.json")
                .remove("type").build();
    ...
    public void toStringTest() {
        JsonObject rawSchemaJson = loader.readJsonObject("tostring/numberschema.json");
        String actual = SchemaLoaderImpl.schemaLoader().readSchema(rawSchemaJson).toString();
        assertEquals(rawSchemaJson, readJsonObject(actual));
    }
