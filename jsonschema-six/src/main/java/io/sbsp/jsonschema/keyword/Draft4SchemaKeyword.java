...
import static io.sbsp.jsonschema.keyword.KeywordMetadata.*;
...
public interface Draft4SchemaKeyword {
     ...
     */
     KeywordMetadata<MaximumKeyword> exclusiveMaximumDraft4 = keyword(MaximumKeyword.class).key("exclusiveMaximum").expects(TRUE, FALSE).validates(JsonSchemaType.NUMBER).from(Draft3).until(Draft5).build();
     ...
     */
    KeywordMetadata<MinimumKeyword> exclusiveMinimumDraft4 = keyword(MinimumKeyword.class).key("exclusiveMinimum").expects(TRUE, FALSE).validates(JsonSchemaType.NUMBER).from(Draft3).until(Draft5).build();
